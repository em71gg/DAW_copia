 ...
 */
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    campeón para el torneo en ese año
    */
    ...
    if ($torneoId !== null && $fechaTorneo !== null){
      ...
      if (empty($campeonFecha)) {//Si no existe ganador para esa consulta se hace el insert
        ...
        if ($resultado){
          ...
          $apellidosCampeon = $resultado['apellidos'];
        } 

        salidaDatos(json_encode(["mensaje" => "No se puede asignar ganador al torneo, en esa fecha fue ganado por $nombreCampeon $apellidosCampeon."]), array('HTTP/1.1 400 Bad Request'));
        exit;
      }
    }
    else {
