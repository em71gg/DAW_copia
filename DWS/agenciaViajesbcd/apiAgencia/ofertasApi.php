...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    } elseif (isset($_GET['usuario_id']) && isset($_GET['array'])) { //consulta para obtener la ofertas por ofertante con array
        $id_usuario = $_GET['usuario_id'];
        //comprobar que el id pertenece a un ofertante
        $checkId = $conexion->prepare("SELECT o.id
                                        FROM ofertas o
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (
        ...
        salidaDatos(
            'Debe completar todos los parámetros de la petición JSON.' . PHP_EOL .
    ...
    if ($checkId->rowCount() == 0) {
        salidaDatos(
            'Identificador de ofertante inadecuado.',
            array('HTTP/1.1 400 Bad Request')
        );
        exit;
    }
...
if ($_SERVER['REQUEST_METHOD'] == 'PUT') {
    ...
    $datos = json_decode(file_get_contents('php://input'), true);
    if (!isset($datos['id'])) {
        salidaDatos('Se necesita un id de la oferta a actualizar. ', array('HTTP/1.1 400 Bad Request'));
        exit;
    }
    $ofertaId = $datos['id'];
