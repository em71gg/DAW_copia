    ...
    if (!empty($email)) {
        if (validarEmail($email)) { //si el email es válio se comprueba si está registrado
            ...
            if ($consulta-> rowCount() > 0) { //Si el email existe en la bbdd llevo  creo sesion y llevo a introducir contraseña
                ...
                $conexiong = conectarPDO($host, $user, $password, $bbdd);
                $consultag = $conexiong->prepare("SELECT * FROM gestores WHERE email=?");
                $consultag->bindParam(1, $email);
                $consultag->execute();

                if ($consultag-> rowCount() > 0) { //Si el email existe en la bbdd llevo  creo sesion y llevo a introducir contraseña
                echo "$email existe en la base de datos";
                echo "Valor email tras crear la sesion: $email";
