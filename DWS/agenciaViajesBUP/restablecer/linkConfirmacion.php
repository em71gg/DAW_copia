...
$hasTokenColumn = false; // Para determinar si la tabla tiene la columna 'token'
...
try {
    ...
    if (!$conexion) {
        ...
        if ($consultaUsuarios->rowCount() > 0) {
            ...
            // Verificar si la tabla 'usuarios' tiene la columna 'token'
            $consultaColumnas = $conexion->query("SHOW COLUMNS FROM usuarios LIKE 'token'");
            $hasTokenColumn = $consultaColumnas->rowCount() > 0;
        } else {
            // Si no está en 'usuarios', buscamos en 'gestores'
            $consultaGestores = $conexion->prepare("SELECT * FROM gestores WHERE email = ?");
            $consultaGestores->bindParam(1, $_SESSION['email']);
            $consultaGestores->execute();
            
            if ($consultaGestores->rowCount() > 0) {
                $tipoUsuario = 'gestores';
                // Verificar si la tabla 'gestores' tiene la columna 'token'
                $consultaColumnas = $conexion->query("SHOW COLUMNS FROM gestores LIKE 'token'");
                $hasTokenColumn = $consultaColumnas->rowCount() > 0;
            } else {
    ...
    if (isset($consultaColumnas)) {
        $consultaColumnas = null;
    }
    if (isset($conexion)) {
        $conexion = null;
    }
    ...
    if (count($errores) == 0) {
        ...
        try {
            ...
            if (!$conexion) {
                ...
                if ($hasTokenColumn) {
                    // Si la tabla tiene la columna 'token', generamos un token y enviamos un correo
                    ...
                    // Si la tabla no tiene la columna 'token' (como 'gestores'), actualizamos la contraseña directamente
                    $passToCrypt = password_hash($_SESSION['password'], PASSWORD_BCRYPT);
                    $updatePass = $conexion->prepare("UPDATE $tabla SET password=?, updated_at=CURRENT_TIMESTAMP WHERE email =?");
                    $updatePass->bindParam(1, $passToCrypt);
                    $updatePass->bindParam(2, $_SESSION['email']);
                    $success = $updatePass->execute();

                    if (!$success) {
                        $errores['execute'] = "Error al ejecutar la consulta de actualización de contraseña.";
                    } else {
                        $rowCount = $updatePass->rowCount();
                        echo "Filas afectadas (contraseña actualizada): $rowCount<br>";

                        if ($rowCount > 0) {
                            // Contraseña actualizada con éxito, redirigimos directamente a login
                            session_destroy();
                            header('refresh:3; url=../acceso/login.php');
                            $errores['success'] = "Contraseña actualizada con éxito. Será redirigido a la pantalla de login.";
                        } else {
                            $errores['update'] = "No se pudo actualizar la contraseña para el email: {$_SESSION['email']}";
                        }
                    }
                }
            }
