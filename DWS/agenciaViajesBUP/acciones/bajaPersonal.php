...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    if (isset($_GET['id'])) {
        $eliminarEmpleado = $conexion -> prepare ('DELETE FROM gestores WHERE id=?');
        $eliminarEmpleado -> bindParam(1, $_GET['id']);
        $eliminarEmpleado -> execute();

        if($eliminarEmpleado -> rowCount() >0) echo "Empleado eliminado con éxito";
    }
}
?>
                                    ...
                                    foreach ($rdo as $campo) {
                                        ...
                                        echo "<tr class='$rowClass hover:bg-gray-200'>
                                            ...
                                            <td class='p-1 sm:p-2 text-gray-800 text-xs sm:text-sm' data-label='Acciones'>
                                                <a href='" . htmlspecialchars($_SERVER['PHP_SELF']) ."?id=" . $campo['ID'] . "' class='text-blue-600 font-bold hover:underline'>Elminar</a>
                                            </td>
                                        </tr>" . PHP_EOL;
                                    }
                                }
            ...
            <div class="p-4 sm:p-7">
                <div class="text-center">
                    ...
                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                        ...
                        <a class="text-blue-600 decoration-2 hover:underline font-medium" href="../areaPersonal/editarAdmin.php">
                            Volver
                        </a>
                    </p>
                </div>
